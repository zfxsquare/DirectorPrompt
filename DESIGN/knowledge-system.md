...
每轮叙事前, Knowledge Agent 用当前场景信息做三路并行检索, 合并去重后注入 Narrator 上下文。
...
状态系统的 Phase 机制在状态值满足条件时持续注入关联的禁用知识。比如天气为“暴风雨”时, Phase 表达式 `{val} == "暴风雨"` 匹配, 系统自动把关联的禁用知识条目注入叙事上下文。
